...
#### Feed Interface (`src/app/core/types/firestore.types.ts`)

```typescript
interface Feed extends FirestoreDocument {
  ...
  type: 'NYC' | 'OTHER'; // Rule type (strict typed)
}
```
...
- **Standalone components (Angular 20)** - No NgModules required
- **Signal-based reactive state** - Primary state management approach
- **Dependency injection with `inject()`** - Modern DI pattern
- **OnPush change detection** for optimal performance
- **Computed signals** for derived state calculations
- **TypeScript strict typing** with interfaces and type guards

### Modern Angular 20 Features Used

- **Control Flow Syntax** - `@if`, `@else if`, `@for` in templates
- **Signal-based reactivity** - `signal()`, `computed()`, `effect()`
- **Standalone architecture** - Components, pipes, directives
- **New lifecycle hooks** - `DestroyRef` for cleanup
...
- **User feedback** via toast notifications and status indicators
- **Network state monitoring** with automatic retry mechanisms

### UI/UX Patterns

- **Loading states** with `IonSkeletonText` components
- **Empty states** with informative messaging
...
1. Services fetch data from Firebase with type safety
2. Components use signals for reactive state management
3. Computed signals derive UI state automatically
4. Loading and error states handled gracefully
5. UI updates reactively through signal changes
6. User actions trigger service calls with proper error handling
...
This documentation provides AI agents with comprehensive understanding of the Paras (ASP NYC) app architecture, features, and development practices.
