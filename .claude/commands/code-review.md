...
- **CLEANUP** — A small, low-risk improvement with a clear mechanical fix (unused param, soft limit breach, dead import,
  rename). Actionable but not urgent. Distinct from NOTED so it doesn't get buried.
- **NOTED** — Pure context: intentionally deferred tech debt, a pattern that's consistent across the codebase, or a
  design decision that was already reasoned through. No action expected.
...
### CLEANUP (<count>) — Small improvements worth doing

- <file>:<line> — <what to do> (one line, actionable)

---

### NOTED (<count>) — Context only, no action needed
...
- CLEANUP items must have a concrete one-line action — if you can't state exactly what to change, it's NOTED
