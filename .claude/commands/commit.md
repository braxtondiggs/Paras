...
- **Describe the change, not its provenance**: Commit messages must describe what changed and why. Do NOT reference the
  source of the change — no "bot review findings", "Copilot/Gemini/Claude suggestions", "PR review comments", "code
  review feedback", "addresses reviewer comment", or "applies suggestions from <tool>". Future readers care about the
  change itself, not how it was surfaced. Reviewer attribution belongs in the PR discussion, not the commit history.
...
Bad commit messages (describe provenance, not the change):

- fix: apply PR review findings before merge
- fix: address bot review comments (Copilot/Gemini/Claude)
- chore: implement reviewer suggestions
- fix: incorporate code review feedback

Rewrite these to describe what actually changed, e.g.:

- fix: correct audit key, stablecoin normalization, and round-robin stamping
- fix: round allocation to 2dp before persistence to match decimal(5,2) column
